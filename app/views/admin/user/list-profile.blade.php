    ...
    <main class="w-full px-3 mx-auto sm:px-5">
        <form method="get" action="{{ route('admin/search-user') }}"
            ...
            <div class="relative flex items-center form-group">
                <button type="submit" class="absolute" name="profile">
                                ...
                                <tr>
                                    <th scope="col"
                                        ...
                                        <div class="flex items-center gap-x-3 focus:outline-none">
                                            <span>Tên</span>
                                            <a
                                                href="{{ !empty($keyword) ? route('admin/search-profile/' . $keyword . '/first_name/ASC') : route('admin/profile/list/first_name/ASC') }}">
                                    ...
                                    <th scope="col"
                                        ...
                                        <div class="flex items-center gap-x-3 focus:outline-none">
                                            <a
                                                href="{{ !empty($keyword) ? route('admin/search-profile/' . $keyword . '/birthday/ASC') : route('admin/profile/list/birthday/ASC') }}">
                                            ...
                                            <span>Ngày sinh</span>
                                            <a
                                                href="{{ !empty($keyword) ? route('admin/search-profile/' . $keyword . '/birthday/DESC') : route('admin/profile/list/birthday/DESC') }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor"
                                ...
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td class="p-4 text-sm font-medium whitespace-nowrap">
                                            <div
                                                class="text-white text-sm font-medium  {{ ($user->gender == 0 ? 'badge badge-secondary' : $user->gender == 1) ? 'badge badge-info' : 'badge badge-primary' }}">
                                                {{ ($user->gender == 0 ? 'Nữ' : $user->gender == 1) ? 'Nam' : 'Khác' }}
                                            </div>
                                        </td>

                                        <td class="py-4 pr-5 text-sm text-center whitespace-nowrap">
                                            <p class="text-gray-700 dark:text-white">
                                                {{ isset($user->birthday) ? date('d/m/Y', strtotime($user->birthday)) : 'Trống' }}
                                            </p>
                                        </td>

                                        <td
                                            class="px-4 py-4 text-sm max-w-[220px] overflow-hidden truncate whitespace-nowrap">
                                            <p class="text-gray-700 dark:text-white">
                                                {{ isset($user->address) ? $user->address : 'Trống' }}
                                            </p>
                                        </td>
                                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                                            <p
                                                class="max-w-[250px] overflow-hidden truncate text-gray-900 dark:text-white">
                                                {{ isset($user->bio) ? $user->bio : 'Trống' }}
                                            </p>
                                        </td>
                                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                ...
                                                <ul tabindex="0"
                                                    ...
                                                    <li class="flex items-center justify-between gap-3">
                                                        <a href="{{ route('admin/profile/' . $user->id) }}"
                                                            class="font-medium hover:text-indigo-500">Xem chi
                                                            tiết</a>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                            class="w-4 h-4">
                                                    ...
                                                    <li class="flex items-center justify-between gap-3">
                                                        <a href="{{ route('admin/profile/update/' . $user->id) }}"
                                                    ...
                                                    <li class="flex items-center justify-between gap-3">
                                                        <a class="font-medium delete-account hover:text-indigo-500"
                                                            href="{{ route('admin/delete-account/' . $user->id) }} "
                                                            data-id="{{ $user->id }}">Xóa tài khoản này</a>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                            class="w-4 h-4">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
