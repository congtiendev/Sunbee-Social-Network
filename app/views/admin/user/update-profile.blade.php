        ...
        <section class="flex w-full h-screen gap-1">
                ...
                <div class="overflow-y-auto bg-white shadow-xs">
                    <ul class="flex flex-col gap-1 p-3 list-sidebar">
                        <li class="flex items-center gap-1 p-2 rounded-md">
                            <a href="{{ route('admin/profile/update/' . $user->id) }}" class="flex items-center gap-2">
                        ...
                        <li class="flex items-center gap-1 p-2 bg-gray-100 rounded-md">
                            <a href="{{ route('admin/account/update/' . $user->id) }}" class="flex items-center gap-2">
            ...
            <article class="w-full h-full p-3 bg-white shadow-xs">
                <section class="w-full h-full setting-user-info">
                    <form action="{{ route('admin/profile/update/handle/' . $user->id) }}" method="post"
                            ...
                            <div class="form-group">
                                ...
                                <div class="relative flex items-center mt-2">
                                    ...
                                    <input type="text" placeholder="Họ..." name="first_name"
                                        ...
                                        value="{{ isset($_SESSION['old']['first_name']) && isset($_GET['msg'])
                                            ? trim($_SESSION['old']['first_name'])
                            ...
                            <div class="form-group">
                                ...
                                <div class="relative flex items-center mt-2">
                                    ...
                                    <input type="text" placeholder="Tên..." name="last_name"
                                        class="block w-full py-2.5 text-gray-700 placeholder-gray-400/70 bg-white border border-gray-200 rounded-lg pl-11 pr-5 rtl:pr-11 rtl:pl-5   focus:border-indigo-400 focus:ring-indborder-indigo-300 focus:outline-none focus:ring focus:ring-opacity-40"
                                        value="{{ isset($_SESSION['old']['last_name']) && isset($_GET['msg'])
                                            ? trim($_SESSION['old']['last_name'])
                            ...
                            <div class="form-group">
                                ...
                                <div class="relative flex items-center mt-2">
                                    ...
                                    <input type="date" name="birthday"
                                        ...
                                        value="{{ isset($_SESSION['old']['birthday']) && isset($_GET['msg'])
                                            ? trim($_SESSION['old']['birthday'])
                                            : (!empty($user->birthday)
                                                ? trim($user->birthday)
                                                : null) }}">
                                </div>
                                <span class="text-xs text-red-500">
                                    @if (isset($_SESSION['errors']) &&
                                            isset($_SESSION['errors']['birthday']) &&
                                            count($_SESSION['errors']['birthday']) > 0 &&
                                            isset($_GET['msg']))
                                        {{ $_SESSION['errors']['birthday'][0] }}
                                    @endif
                                </span>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="relative flex items-center mt-2">
                                    ...
                                    <input type="text" placeholder="Địa chỉ..." name="address"
                                        class="block w-full py-2.5 text-gray-700 placeholder-gray-400/70 bg-white border border-gray-200 rounded-lg pl-11 pr-5 rtl:pr-11 rtl:pl-5   focus:border-indigo-400 focus:ring-indborder-indigo-300 focus:outline-none focus:ring focus:ring-opacity-40"
                                        value="{{ isset($_SESSION['old']['address']) && isset($_GET['msg'])
                                            ? trim($_SESSION['old']['address'])
                                            : (!empty($user->address)
                                                ? trim($user->address)
                                                : null) }}">
                                </div>
                                <span class="text-xs text-red-500">
                                    @if (isset($_SESSION['errors']) &&
                                            ...
                                            count($_SESSION['errors']['address']) > 0 &&
                                            isset($_GET['msg']))
                        ...
                        <div class="flex flex-col gap-2 mt-5 form-group"><label for="first_name"
                                class="block text-sm font-medium text-gray-500 ">Tiểu
                                sử
                            </label>
                            <textarea name="bio" id="bio"
                                ...
                                placeholder="Nhập tiểu sử...">{{ isset($_SESSION['old']['bio']) && isset($_GET['msg'])
                                    ? trim($_SESSION['old']['bio'])
                                    : (!empty($user->bio)
                                        ? trim($user->bio)
                                        : null) }}</textarea>
                            <span class="text-xs text-red-500">
                                @if (isset($_SESSION['errors']) &&
                                        isset($_SESSION['errors']['bio']) &&
                                        count($_SESSION['errors']['bio']) > 0 &&
                                        isset($_GET['msg']))
                                    {{ $_SESSION['errors']['bio'][0] }}
                                @endif
                            </span>
                        </div>
